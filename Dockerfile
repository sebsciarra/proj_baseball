...
# and `pip install proj_baseball` using an artifact store. Faster and more robust.
...
SHELL ["mamba", "run", "-n", "proj_baseball", "/bin/bash", "-c"]

# Build package
RUN tox -e build
RUN mamba env export -n proj_baseball -f environment.docker.yml
...
SHELL ["conda", "run", "-n", "proj_baseball", "/bin/bash", "-c"]
...
# ENTRYPOINT ["conda", "run", "--no-capture-output", "-n", "proj_baseball", "YOUR_CONSOLE_SCRIPT"]
